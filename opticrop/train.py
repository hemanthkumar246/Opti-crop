...
#Note on paths: this script assumes the folder layout from the zip — data/Crop_recommendation.csv sits one level up from model/train_model.py. Run it as python3 model/train_model.py from the opticrop/ root, or adjust BASE_DIR if you place it elsewhere. Let me know if you'd like the code for app.py or the HTML templates shown the same way.